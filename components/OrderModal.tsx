...
export const OrderModal: React.FC<OrderModalProps> = ({ isOpen, onClose, onSave, onDelete, draftOrder, editingOrder, vets, isSubmitting, activeSoapId, clinicSettings }) => {
    ...
    if (isOpen) {
      ...
      if (editingOrder) { 
        ...
        const normalizedImages = (editingOrder.images || []).map((img: any) => {
            if (typeof img === 'string') return { url: img, name: 'Attached Image' };
            return img;
        });
      ...
      <div className="bg-white rounded-xl w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            {isCompleted && (
              <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-emerald-200 shadow-sm">
                Archived Record
              </span>
            )}
            {!activeSoapId && !editingOrder?.soap_id && (
          ...
          <div className="flex items-center gap-3">
            {isUploading && <span className="text-[10px] font-bold text-blue-600 animate-pulse uppercase tracking-widest">Processing Media...</span>}
        ...
        <div className="bg-white border-b border-slate-100 p-1.5 flex gap-1">
          {DEPARTMENTS.map(dept => (
            <button 
              ...
              className={`flex-1 py-2 px-2 rounded-lg flex items-center justify-center gap-2 border transition-all ${
            ...
            >
              ...
              <span className="text-[11px] font-black uppercase">{dept.label}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
           <div className={`w-1/3 p-5 border-r border-slate-100 flex flex-col bg-slate-50/50`}>
             <div className="relative mb-3">
               <input 
                ...
                className="w-full bg-white border border-slate-200 rounded-lg pl-8 pr-3 py-2 text-xs outline-none focus:border-blue-500 shadow-sm" 
              />
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar bg-white border border-slate-200 rounded-lg relative shadow-inner">
                {catalog.filter(i => i.name.toLowerCase().includes(searchTerm.toLowerCase())).map(item => (
                  <div 
                    key={item.id} 
                    onClick={() => { 
                      if(item.category==='PHARMACY') setCalculatingItem(item); 
                      else setSelectedItems(p => [...p, { id: Date.now().toString(), service_id: item.id, name: item.name, unit_price: item.default_price, quantity: 1, total_price: item.default_price, category: item.category, performingVetId: vetId }]); 
                    }} 
                    className="px-3 py-2 border-b border-slate-50 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors"
                  >
                    <span className="text-xs font-bold truncate">{item.name}</span>
                    <span className="text-[10px] text-slate-400 font-bold font-mono">₩{item.default_price.toLocaleString()}</span>
                  </div>
                ))}
                {calculatingItem && (
                  <QuantityCalculator 
                    item={calculatingItem} 
                    onConfirm={q => { 
                      setSelectedItems(p => [...p, { id: Date.now().toString(), service_id: calculatingItem.id, name: calculatingItem.name, unit_price: calculatingItem.default_price, quantity: q, total_price: calculatingItem.default_price * q, category: calculatingItem.category, performingVetId: vetId }]); 
                      setCalculatingItem(null); 
                    }} 
                    onCancel={() => setCalculatingItem(null)} 
                  />
                )}
             </div>
           </div>
           ...
           <div className="w-1/3 p-5 flex flex-col space-y-4 border-r border-slate-100 overflow-y-auto">
              <div className="bg-blue-50/50 p-2.5 rounded-lg border border-blue-100 flex items-center justify-between">
                ...
                {isCompleted && <i className="fas fa-check-circle text-emerald-500 text-[10px]"></i>}
              </div>
              
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Order Practitioner</label>
                <select 
                  value={vetId} 
                  onChange={e => setVetId(e.target.value)} 
                  className={`w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold outline-none focus:border-blue-500 transition-all`}
                >
              ...
              <div className="flex-1 min-h-[180px] border border-slate-200 rounded-lg flex flex-col bg-white overflow-hidden shadow-inner">
                <div className="bg-slate-100 px-3 py-1.5 border-b border-slate-200 flex justify-between items-center">
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Selected Items ({selectedItems.length})</span>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  {selectedItems.map((it, idx) => (
                    <div key={it.id} className="flex justify-between items-center px-3 py-2 border-b border-slate-50 last:border-0 group">
                      <span className="text-[10px] font-black text-slate-800 truncate flex-1 uppercase tracking-tight">{it.name} x{it.quantity}</span>
                      <button type="button" onClick={() => setSelectedItems(p => p.filter((_,i)=>i!==idx))} className="text-slate-300 hover:text-rose-500 ml-2 transition-colors">
                  ...
                  {selectedItems.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center opacity-20 py-10">
                      ...
                      <p className="text-[9px] font-black uppercase">Order List is Empty</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center ml-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Order Instruction</label>
                    {isCompleted && <span className="text-[8px] font-black text-amber-600 bg-amber-50 px-1.5 rounded uppercase border border-amber-100">Locked</span>}
                </div>
                <textarea 
                  value={details} 
                  readOnly={isCompleted}
                  onChange={e => setDetails(e.target.value)} 
                  placeholder="Additional instructions for treatment or pharmacy..." 
                  className={`w-full h-28 border rounded-xl px-4 py-3 text-sm font-bold outline-none resize-none shadow-inner leading-relaxed transition-all ${
                    isCompleted 
           ...
           <div className="w-1/3 p-5 flex flex-col bg-slate-50/30">
              <div className="flex justify-between items-center mb-3">
                ...
                <button 
                  ...
                  className="px-3 py-1 bg-slate-900 text-white rounded-lg text-[9px] font-black uppercase shadow-sm hover:bg-black transition-all"
                >
                  Upload
                </button>
              ...
              <div className={`flex-1 overflow-y-auto custom-scrollbar border-2 border-dashed rounded-2xl p-3 bg-white shadow-inner border-slate-200`}>
                  ...
                  return (
                    <div key={idx} className={`relative group bg-white border rounded-xl overflow-hidden mb-3 shadow-sm hover:shadow-md transition-all ${isError ? 'border-rose-300' : isTailscale ? 'border-blue-500 ring-2 ring-blue-500/20 shadow-blue-500/10 cursor-zoom-in' : 'border-slate-200 opacity-60'}`}>
                      {isError ? (
                        ...
                        <div className="w-full aspect-square flex flex-col items-center justify-center bg-slate-50 text-slate-400 gap-2">
                          <i className="fas fa-sync fa-spin text-lg text-blue-500"></i>
                          <span className="text-[8px] font-black uppercase tracking-widest">Syncing to NAS...</span>
                        </div>
                      )}
                      
                      <button type="button" onClick={() => setOrderImages(p => p.filter(x => x.url !== img.url))} className="absolute top-2 right-2 w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10 shadow-lg">
                ...
                {orderImages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center opacity-10 py-20 pointer-events-none">
                    <i className="fas fa-images text-3xl mb-4"></i>
                    <p className="text-[10px] font-black uppercase">No Assets Attached</p>
                  </div>
                )}
              </div>
           </div>
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 items-center">
          {(!activeSoapId && !editingOrder?.soap_id) && (
            <span className="text-[10px] font-black text-rose-500 uppercase tracking-tight italic mr-auto">
              <i className="fas fa-exclamation-triangle mr-1"></i> Chart must be saved before sending orders
            </span>
          )}
          {editingOrder && onDelete && (
            <button 
              type="button" 
              onClick={(e) => { 
                e.stopPropagation(); 
              ...
              className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${
                confirmDelete 
                  ? 'bg-rose-600 text-white shadow-xl animate-pulse ring-4 ring-rose-200' 
          ...
          {isCompleted ? (
            <button 
              ...
              onClick={() => handleFinalSave('Completed')} 
              className="px-8 py-2.5 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 active:scale-95 disabled:opacity-50 transition-all"
            >
              Update Archived Order
            </button>
          ) : (
            <>
              <button 
              ...
              <button 
                type="button" 
                disabled={isSubmitDisabled} 
                onClick={() => handleFinalSave(editingOrder?.status || 'Pending')} 
                className="px-8 py-2.5 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-slate-900/20 hover:bg-black active:scale-95 disabled:opacity-50 transition-all"
              >
                Save Draft Order
              </button>
            </>
